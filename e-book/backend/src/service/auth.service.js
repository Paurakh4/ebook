...
export const registerService = async (name, email, password) => {
  try {
    ...
    // Validate Gmail existence (check if it is a gmail address)
    if (!email || !email.endsWith("@gmail.com")) {
      throw new AppError("Unregistered email", 400);
    }
...
export const googleIdTokenService = async (idToken) => {
  try {
    ...
    if (!user) {
      user = await userModel.create({ name, email, isGoogle: true, picture: picture || "" });
