...
import { login as loginService, register, forgotPassword, resetPassword, googleTokenLogin } from "../components/services/authServices";
...
export default function Login() {
  ...
  useEffect(() => {
    if (response?.type === 'success') {
      ...
      if (__DEV__) console.log('Google Auth ID token received:', idToken ? 'YES' : 'NO');
      if (idToken) {
      ...
      if (__DEV__) console.log('Google Auth cancelled by user.');
    }
  }, [response]);
  ...
  const handleGoogleLogin = async (idToken: string) => {
    setIsLoading(true);
    if (__DEV__) console.log('Google Login: verifying token with backend...');
    try {
      const res = await googleTokenLogin(idToken);
