...
} from '../components/services/subscriptionServices';
    ...
    const handleSubscribe = async () => {
        ...
        try {
            ...
            if (!prepareRes.success || !prepareRes.data) {
                Alert.alert("Error", prepareRes.message || "Failed to start subscription");
                return;
            }

            if (prepareRes.data.alreadySubscribed) {
                const profileRes = await getProfile();
            ...
            const initRes = await initPaymentSheet({
                ...
                customerId: prepareRes.data.customerId,
                customerEphemeralKeySecret: prepareRes.data.ephemeralKey,
                paymentIntentClientSecret: prepareRes.data.clientSecret,
                returnURL: prepareRes.data.returnUrl,
            ...
            if (initRes.error) {
                Alert.alert("Error", initRes.error.message || "Unable to initialize Stripe checkout");
                return;
            }
            ...
            if (paymentRes.error) {
                if (paymentRes.error.code !== "Canceled") {
                    Alert.alert("Payment Failed", paymentRes.error.message || "Subscription was not completed");
                }
                return;
            }
