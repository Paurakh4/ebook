...
`;
    ...
    if (isSpeaking && !isPaused) { await Speech.pause(); setIsPaused(true); }
      ...
      webViewRef.current?.injectJavaScript(`
        (function(){
          ...
          var text=sel||(function(){var tl=document.querySelectorAll('.textLayer');return tl.length>0?Array.from(tl).map(function(l){return l.textContent||l.innerText;}).join('\\n'):(document.body.innerText||document.body.textContent);})();
  ...
  const startSpeech = (textToRead: string, startFrom: number, speakRate: number) => {
    ...
    // Chunking text to avoid Android TTS limit (~4000 characters)
    ...
    while (i < rem.length) {
      ...
      if (end < rem.length) {
        let lastSpace = rem.lastIndexOf(" ", end);
        let lastStop = rem.lastIndexOf(".", end);
        if (lastStop > i + CHUNK_SIZE / 2) {
          end = lastStop + 1;
        } else if (lastSpace > i + CHUNK_SIZE / 2) {
          end = lastSpace + 1;
        }
      } else {
        end = rem.length;
      }
      chunks.push({ text: rem.substring(i, end), offset: startFrom + i });
      i = end;
    }
    ...
    chunks.forEach((chunk, index) => {
      Speech.speak(chunk.text, {
        rate: speakRate,
        onBoundary: (b: any) => setCurrentCharIndex(chunk.offset + (b.charIndex || 0)),
        onDone: () => {
          if (index === chunks.length - 1) {
            setIsSpeaking(false); setIsPaused(false); setCurrentCharIndex(0); setFullText("");
          }
        },
        onError: () => {
          if (index === chunks.length - 1) {
