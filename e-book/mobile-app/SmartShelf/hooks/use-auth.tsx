...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    const updateUser = async (updatedData: any) => {
        try {
            ...
            // 1. Update in-memory state FIRST so UI remains functional
            setUser(newUser);
            
            // 2. Attempt to persist to storage
            ...
            // Use warn instead of error to prevent intrusive redbox overlays for external environment issues like "Disk Full"
            console.warn('⚠️ [Auth updateUser] Persistent storage failed (e.g. disk full), but session updated in memory:', error);
        }
    };
